...
def main():
    ...
    train_x = list(range(1, len(model.train_losses) + 1))
    val_x = [5 * (i+1) for i in range(len(model.val_losses))]
    ...
    plt.plot(train_x, train_g_loss, label='Train G')
    ...
    plt.plot(train_x, train_d_loss, label='Train D')
